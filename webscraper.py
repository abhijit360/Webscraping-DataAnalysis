...
def formatData(data):
    ...
    has_header = False
    try:
        with open('AthleteDATA.csv', "r") as file:
            has_header = csv.Sniffer().has_header(file.read(100))
    except: 
        print("facing an error with file!")

    with open('athleteDATA.csv', "a", newline="") as file:
...
with open(html_doc, 'r', encoding="utf-8") as html:
    ...
    with open('athleteDATA.csv', "a", newline="", encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=headers)
        if not has_header:
            writer.writeheader()
    ...
    dataFrame = pandas.read_csv("athleteDATA.csv")
    dataFrame.to_excel("AthleteData.xlsx")
